...
function Landing() {
  useEffect(() => {
    document.title = 'AppsUtk';

    // Remove favicon on landing
    document.querySelectorAll("link[rel~='icon']").forEach((el) => el.remove());
  }, []);
  return (
    <div className="min-h-screen bg-white flex items-center justify-center">
      <div className="flex flex-col md:flex-row flex-wrap items-center justify-center gap-12 md:gap-16 px-4">
        {/* Viral Recipes App */}
        <Link 
          to="/viral-recipes" 
        ...
        <Link 
          ...
          className="flex flex-col items-center space-y-4 group cursor-pointer transition-transform hover:scale-105"
        >
          <div className="w-32 h-32 md:w-40 md:h-40 rounded-3xl shadow-lg group-hover:shadow-xl transition-shadow overflow-hidden bg-black">
            <img 
              src="/ChestMaxx.png" 
              alt="ChestMaxx" 
              className="w-full h-full object-cover" 
